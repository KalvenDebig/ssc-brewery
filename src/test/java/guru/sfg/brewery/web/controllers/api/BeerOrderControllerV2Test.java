...
class BeerOrderControllerV2Test extends BaseIT {
    ...
    @Test
    void listOrderAdmin() throws Exception {
        mockMvc.perform(get(API_ROOT))
    ...
    @Test
    void listOrderCustomerKeyWest() throws Exception {
        mockMvc.perform(get(API_ROOT))
                .andExpect(status().isOk());
    }
    ...
    @Transactional
    @Test
    void getOrderNotAuth() throws Exception {
    ...
    @WithUserDetails(DefaultBreweryLoader.ST_PETE_USER)
    @Test
    void getOrderCustomerAuth() throws Exception {
        BeerOrder beerOrder = stPeteCustomer.getBeerOrders().stream().findFirst().orElseThrow();
    ...
    @WithUserDetails(DefaultBreweryLoader.KEY_WEST_USER)
    @Test
    void getOrderCustomerForbidden() throws Exception {
        BeerOrder beerOrder = stPeteCustomer.getBeerOrders().stream().findFirst().orElseThrow();
